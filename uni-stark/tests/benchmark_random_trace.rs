...
fn benchmark_random_trace() {
    ...
    for log_n in 16..20 {
        ...
        // Generate a random trace instead of a Fibonacci trace
        let rng = thread_rng();
        let poly = rng.sample_iter(Standard).take(n).collect_vec();
        let trace = RowMajorMatrix::new(poly, 2);
        ...
        let _proof = prove(&config, &FibonacciAir {}, &mut challenger, trace, &pis);
        println!("");
    }
}
